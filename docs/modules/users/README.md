...
Текущее ограничение: входящее взаимодействие проверяется относительно `lastSeenAt`, но ещё не относительно `blockedAt`. Защита от запоздавшего входящего события, предшествующего блокировке бота, пока не реализована.
...
Тест совместимости создаёт настоящую Core identity с UUIDv4 и проверяет, что блокировка и обезличивание профиля сохраняют её provider и provider client ID. Общий `composer test:unit` включает существующие тесты Core и проходит вместе с тестами Users.
