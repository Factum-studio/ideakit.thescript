...
> ***Примечание:*** *прямые коммиты разрешены для начальной настройки `репозитория`/`фрэймворка`/`структуры` проекта* 
...
# Или через merge (если вам нужен явный коммит синхронизации)
git merge dev
```
...
- PR **желательно** делать оптимального для ревью размера в районе 400-800 строк кода

### Процесс ревью
1. Ревьюеры проверяют:
    - Соответствие архитектуре (чистая архитектура, DDD, CQRS)
    - Качество кода
    - Покрытие тестами
...
**Важно:** Все изменения, включая документацию, проходят одинаковый процесс ревью.  
Если вы правите `./docs/` – создавайте отдельную ветку и PR, как для кода.
