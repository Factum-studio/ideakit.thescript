...
До появления нового контейнерного окружения можно установить зависимости и проверить требования базового Yii2-приложения в среде с утверждёнными версиями PHP и Composer:
...
Основным способом локального запуска будет Docker Compose с PHP-FPM, Nginx, PostgreSQL, Redis и RabbitMQ. Конфигурация будет добавлена следующим этапом и после проверки описана в корневом README. Встроенный сервер Yii и удалённая PHP 7.4/Apache-конфигурация не являются утверждённым окружением проекта.

Актуальные версии компонентов перечислены в [корневом README](../../README.md#утверждённая-матрица-runtime). Реальные секреты и локальный `.env` в репозитории не хранятся.
