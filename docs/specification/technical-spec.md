...
- ограничение PHP — `~8.4.0`, проверяемое на PHP 8.4.23;
...
Фактические версии реализованной части определяются исполняемыми источниками: `composer.json`, `composer.lock` и конфигурацией CI. Утверждённая матрица runtime приведена в [корневом README](../../README.md#утверждённая-матрица-runtime); контейнерные образы будут добавлены вместе с полноценным окружением.

### 2.2. Целевая конфигурация MVP

| Область | Решение | Правило фиксации |
|---|---|---|
| Язык | PHP 8.4 | Composer допускает `~8.4.0`, совместимость проверяется на PHP 8.4.23 и не заявляется для PHP 8.5 |
...
Полная конфигурация PHP-FPM, Nginx, PostgreSQL, Redis, RabbitMQ и Docker Compose ещё не реализована и не считается действующим локальным окружением.
