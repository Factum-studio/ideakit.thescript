...
Фактические версии реализованной части определяются исполняемыми источниками: `composer.json`, `composer.lock`, Dockerfile, `docker-compose.yml` и конфигурацией CI. Утверждённая матрица runtime и проверенные команды локального запуска приведены в [корневом README](../../README.md#утверждённая-матрица-runtime).

### 2.2. Целевая конфигурация MVP

| Область | Решение | Правило фиксации |
|---|---|---|
| Язык | PHP; нижняя граница 8.1 | изменение версии выполняется отдельной задачей после проверки совместимости |
...
Локальный Docker Compose запускает PHP-FPM, Nginx, PostgreSQL, Redis и RabbitMQ. Прикладная схема и миграции PostgreSQL, интеграция Redis, RabbitMQ topology и фоновые процессы ещё не реализованы.
...
|---|---|---|
| `Users` | бизнес-модуль | пользователи (включая администраторов), их идентификации (Telegram, внутренний сервис, Google и др.), сессии, доступность бота, обезличивание, роли и MFA |
...
![Физическая модель PostgreSQL](../.images/technical-spec-database-erd-v2.png)

Исходник диаграммы: [`database-erd.puml`](../.diagrams/technical-spec-database-erd-v2.puml).

### 11.1. Общие правила

- Основные ключи бизнес-сущностей — UUIDv7, созданные приложением.
...
|---:|---|---|---|
|  1 | `user` | `Users` | основная учётная запись пользователя, роли, статус, контактные данные; id – UUIDv7 |
|  2 | `user_identity` | `Users` | внешние идентификаторы пользователя (Telegram, внутренний, Google и т.п.); id – UUIDv7, user_id – UUIDv7 (FK) |
|  3 | `telegram_bot_sessions` | `Users` | ограниченное состояние диалога и временный черновик заявки |
|  4 | `telegram_updates` | `Telegram` | надёжный входящий контур и защита от повторной обработки |
|  5 | `business_ideas` | `IdeaCatalog` | постоянная идентичность идеи |
|  6 | `idea_versions` | `IdeaCatalog` | неизменяемая редакция карточки и отчёта |
|  7 | `idea_publications` | `IdeaCatalog` | публикация редакции и её место в общей последовательности |
|  8 | `idea_impressions` | `IdeaFeed` | подтверждённый показ и защита от повторной выдачи |
|  9 | `user_idea_actions` | `IdeaFeed` | содержательные действия пользователя с идеей |
| 10 | `agency_leads` | `AgencyLeads` | заявка по конкретной редакции идеи |
| 11 | `agency_lead_status_history` | `AgencyLeads` | неизменяемая история переходов заявки |
| 12 | `agency_lead_messages` | `AgencyLeads` | ограниченная переписка внутри заявки |
| 13 | `outbox_messages` | `Platform` | исходящие сообщения RabbitMQ и Telegram, включая состояние доставки |
| 14 | `ai_jobs` | `ContentGeneration` | авторитетное состояние фоновой операции подготовки контента |
| 15 | `market_source_snapshots` | `IdeaCatalog` | неизменяемый снимок доказательства и состояние повторной проверки |
| 16 | `source_references` | `IdeaCatalog` | связь утверждения отчёта с конкретным снимком источника |
...
- Администратор - пользователь с ролью (SUPER_ADMIN, CONTENT_MANAGER, AGENCY_OPERATOR, VIEWER) в таблице user. Аутентификация по email и паролю, обязательный TOTP MFA и хешированные коды восстановления..
